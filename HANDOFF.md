...
- **ResultSet/Stats/Status**: Identical, Missing, Extra, SizeMismatch, ContentMismatch, ReadError, AccessDenied. Identici solo contati; `problems` = voci non identiche (memoria limitata). Ogni `FileResult` porta anche `fullPath` (radice + relativo, via `pathutil::MakeAbsolute`).
