...
- **GUI risultati**: la lista mostra il percorso completo (`FileResult.fullPath`) e ha una
  scrollbar laterale trascinabile col mouse (drag del thumb, click sulla track, rotella).
  Regole `fullPath`: Missing → source (in offline la root registrata nello snapshot),
  Extra → dest, SizeMismatch → dest, ContentMismatch → source live / dest offline,
  ChangedDuringScan → dest se ha cambiato la destinazione altrimenti source,
  errori hash/enumerazione → lato fallito se uno solo, altrimenti dest (fallback).
  Le righe filtrate sono in cache (`filteredCache_`, rebuild solo a risultati/filtro nuovi).
...
- **ResultSet/Stats/Status**: Identical, Missing, Extra, SizeMismatch, ContentMismatch, ReadError, AccessDenied. Identici solo contati; `problems` = voci non identiche (memoria limitata). Ogni `FileResult` porta anche `fullPath` (radice + relativo, via `pathutil::MakeAbsolute`): Missing → `sourceRoot_` (in offline la root dello snapshot, display-only),   Extra/SizeMismatch → `destRoot_`, ContentMismatch → source live / dest offline, ChangedDuringScan → dest se ha cambiato la destinazione altrimenti source (dest se entrambi), errori hash/enumerazione → solo lato fallito se è uno solo, altrimenti dest come fallback deterministico, errori di sistema senza path → vuoto.
- **CLI** (`main_cli.cpp`): `--source --dest --mode presence|size|content --case-sensitive --enum auto|win32|mft --list-problems --limit --progress --verify-percent <1-100> --verify-pattern edges|center|random`. Usa `main()` + `CommandLineToArgvW` (NON `wmain`, che su MinGW richiede `-municode` → deliberatamente evitato per compatibilità MSVC/MinGW).
...
- **(F2) GUI SDL3** (`src/UI/AppUI.{h,cpp}`, `UI/Utf.{h,cpp}`, `main_gui.cpp`): finestra SDL3, font via SDL3_ttf, campi Sorgente/Destinazione a testo, scelta **back-end** (Auto/Win32/MFT), slider di verifica come unico selettore Size/Content (thread: Auto da IoClass, override solo da CLI `--threads`) (mode/case GUI rimossi: `Presence` e case-sensitive solo da CLI/motore), bottoni AVVIA/INTERROMPI, riga di stato, barra di avanzamento indeterminata, filtri risultati (Tutti/Identici/Mancanti/Extra/Dimensione/Contenuto/Errori/**Tempistiche**: vista directory più lente da `ScanReport::dirTiming` con selettore A/B, pannelli Enumerazione/Hash affiancati o impilati, nota top-N stimata, `Hash cache hit` globale, spiegazione A-da-snapshot in offline; sezione live `MatchTable` (barre A/B a scala dinamica `NiceCeil` su picchi, tacca soglia, picchi, stato backpressure da `ScanProgress`; disegno dedicato `DrawTimings`, mai `DrawResultsList`), lista risultati con scroll (rotella + scrollbar laterale trascinabile: drag thumb / click track) e righe alternate, righe con percorso completo (`fullPath`) e nome file in grassetto (faux-bold, solo ultima componente), right-click su riga → menu "Apri A/B in Esplora risorse" (solo lati esistenti, `explorer /select`) + "Riscansiona" (riverifica live del singolo file con le impostazioni GUI correnti via `SingleVerify`+worker dedicato in `ScanOrchestrator`, riga/stats aggiornate senza rescansione; metadata error conserva le size del lato sano, worker con try/catch → `ReadError` interno mai silenzioso), righe filtrate in cache (`filteredCache_`, mai rescansione per frame), barra riepilogo. Modello di threading: UI sul main thread, scan su worker; progress+risultato copiati sotto mutex. `run()` fa `join()` del worker al close (niente detach → niente use-after-free).
...
- **(F5) Cache hash** (`Hashing/HashCache.{h,cpp}`): chiave `(path assoluto, size, mtime, percent/pattern effettivi)` separata da `\x01` (pattern normalizzato a Edges su percent 100 dentro `MakeKey`); file binario magic `0x43485642` v2 (v1 rifiutata una volta e ricostruita: le chiavi vecchie non matcherebbero mai); chiave ricavata sul file corrente prima del lookup (hit sempre valido); cache corrotta ignorata con messaggio, mai bloccante. `--hash-cache <file>`. I hit sono contati con `std::atomic` nei worker (`ScanReport.hashCacheHits`).
- **(F5) Export** (`Export/`): `ExportUtil` (token italiani `IDENTICO/MANCANTE/EXTRA/DIM_DIVERSA/CONTENUTO_DIVERSO/ERRORE_LETTURA/ACCESSO_NEGATO/MODIFICATO_DURANTE_SCAN`, `CsvEscape` RFC 4180, `JsonEscape` RFC 8259, `HexDigest`, `InferFormat` da estensione). `CsvExporter`: UTF-8 **con BOM** (Excel), colonne `status,path,size_source,size_destination,hash_source,hash_destination`. `JsonExporter`: array in streaming, nessuna BOM; overload con sezione `slowest_dirs` (`list_a/walk_a/list_b/walk_b/hash_a/hash_b` + `hash_cache_hits` globale). `--export <file>` + `--export-format csv|json`. GUI: pulsanti **SNAPSHOT** e **ESPORTA CSV** con dialoghi di salvataggio (IFileSaveDialog/COM).
- **Directory più lente** (`Profiling/DirTiming.{h,cpp}`, top 15 per colonna/lato in CLI + JSON): `listSeconds` = syscall FindFirst/FindNext per dir (hook in `Win32Enumerator`, misura SOLO le syscall mai le callback che possono bloccarsi su backpressure hash); `walkSeconds` = resolve $I30 + walk-step MFT (solo path `EmitMft`, fallback coperto dall'hook Win32 con prefisso `relPrefix`); `hashSeconds` = somma `FileTimings.totalTicks` per dir padre via `DirHashTop` Space-Saving (64 contatori/lato; mutex per update inclusa scansione del minimo; cache hit esclusi = ~zero, conteggio globale). `list`/`walk` = top-N esatte; `hash` = top-N stimata (ordinamento per stima, può differire per valori vicini; bound intenzionale). Mai sommare colonne. `ScanReport.dirTiming` (plain data via `SnapshotRunTiming`); niente timing in snapshot; offline = lato A vuoto (indice, non filesystem). Plumbing: `IFileEnumerator::setDirListSink` (default no-op), `RunDirTiming` in `ScanController` → `ConcurrentComparer`, sink opzionali in `HashOneSide`/`HashSourceIndex`/`SubmitHashCandidates`/`RunHashPhase`.
...
14. **Verifica parziale** (`Hashing/PartialRead`, `ContentVerifyLevel` in `ScanMode.h`): piano puro intero (mai float) calcolato UNA volta per file da `sizeSource` in `HashOneCandidateInto`, applicato identico ad A/B; Random risolto una volta per run in `ScanController::run`; capture/offline sempre full (le digest archiviate sono complete); chiave cache con livello effettivo + normalizzazione 100/Edges in `MakeKey` (versione cache v2); `IdenticalPartial` solo contato (`identicalPartialFiles`), mai in `problems`; `names[]` della CLI indicizzato per valore enum — reinserire un valore in mezzo all'enum lo rompe silenziosamente (preso dai test: aggiornare array + guardia bounds).
14. **`do/while` → `while` con `continue`**: convertire `do { ...; if (x) continue; } while (ADVANCE())` in `while (cond) { ...; if (x) continue; ...; ADVANCE(); }` crea un loop infinito, perché `continue` salta l'avanzamento (nel `do/while` saltava alla condizione che avanzava). **FIX**: mettere l'avanzamento in TESTA al loop (`for(;;)` con `if (!first) { if (!ADVANCE()) break; }`), così `continue` avanza sempre. (Preso in `Win32Enumerator`: hang al 100% CPU su ogni directory per `.`/`..`.)
