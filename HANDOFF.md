...
- **Directory più lente** (`Profiling/DirTiming.{h,cpp}`, top 15 per colonna/lato in CLI + JSON): `listSeconds` = syscall FindFirst/FindNext per dir (hook in `Win32Enumerator`, misura SOLO le syscall mai le callback che possono bloccarsi su backpressure hash); `walkSeconds` = resolve $I30 + walk-step MFT (solo path `EmitMft`, fallback coperto dall'hook Win32 con prefisso `relPrefix`); `hashSeconds` = somma `FileTimings.totalTicks` per dir padre via `DirHashTop` Space-Saving (64 contatori/lato, 1 mutex; cache hit = ~zero, conteggio globale). Mai sommare colonne. `ScanReport.dirTiming` (plain data via `SnapshotRunTiming`); niente timing in snapshot; offline = lato A vuoto. Plumbing: `IFileEnumerator::setDirListSink` (default no-op), `RunDirTiming` in `ScanController` → `ConcurrentComparer`, sink opzionali in `HashOneSide`/`HashSourceIndex`/`SubmitHashCandidates`/`RunHashPhase`.
